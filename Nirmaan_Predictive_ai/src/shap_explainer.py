...
Supports optional pre-built explainer caching for fast API responses.
"""
...
FEATURE_DISPLAY_NAMES = {
    ...
    # Engineered features
    "cost_per_day": "Daily project cost intensity",
    "progress_gap": "Schedule deviation (behind/ahead)",
    "sanction_ratio": "Funding adequacy ratio",
    "remaining_days_pct": "Remaining timeline percentage",
}


def build_explainer(model) -> shap.TreeExplainer:
    """
    Build a SHAP TreeExplainer for a given model.
    Call this once at startup and reuse the explainer for all predictions.
...
def get_top_factors(
    ...
    Args:
        ...
        explainer: Optional pre-built TreeExplainer (avoids rebuilding per-request).
    ...
    try:
        ...
        sample_shap = (
            np.abs(shap_values[0]) if shap_values.ndim > 1 else np.abs(shap_values)
        )
        ...
        return factors if factors else ["unknown_factor"]

    except Exception as e:
        # Graceful fallback — never crash the API due to SHAP
        print(f"[SHAP WARNING] Could not compute explanations: {e}")
        return ["could_not_compute"] * min(top_n, 1)
...
def get_full_explanation(
    ...
    Args:
        ...
        explainer: Optional pre-built TreeExplainer.
    ...
    try:
        ...
        print(f"[SHAP WARNING] Could not compute full explanation: {e}")
        return {
            "base_value": 0.0,
            "factors": [
                {
                    "feature": "unavailable",
                    "display_name": "Could not compute explanation",
                    "shap_value": 0.0,
                    "abs_impact": 0.0,
                    "direction": "unknown",
                }
...
if __name__ == "__main__":
    ...
    print("         Use build_explainer() to create a cached explainer.")
