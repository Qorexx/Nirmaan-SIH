...
import SandboxTester from './components/SandboxTester.jsx';
import ApiJsonViewer from './components/ApiJsonViewer.jsx';
...
export default function App() {
  ...
  const fetchDuplicates = async () => {
    ...
    try {
      ...
      setError('Cannot connect to Duplicate Detection AI API backend');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
              ...
              <span className="text-2xl font-black text-slate-900">
                {apiData?.total_projects_scanned || 10}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {error && (
              <div className="bg-red-50 rounded-3xl p-8 border border-red-200 text-center space-y-3">
                ...
                <p className="text-xs text-red-700">Ensure backend server is running on port 5000 (`node server/index.js`).</p>
                <button
      ...
      <footer className="bg-white border-t border-slate-200 py-6 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-3">
          <div className="flex items-center space-x-2">
            <span className="font-extrabold text-slate-900">Person 3 AI Module</span>
            <span>— Duplicate & Similarity Detection for MPLADS Scheme</span>
          </div>
          <div>Feeds structured risk metrics directly to Person 6's Risk Engine.</div>
        </div>
      </footer>
