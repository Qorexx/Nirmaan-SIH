/**
 * Person 3 — Duplicate & Similarity Detection Engine (JS Math Fallback)
 * 
 * Multi-factor math engine for MPLADS Scheme.
 * Computes pure mathematical Jaccard token similarity, Haversine spatial proximity,
 * category taxonomy overlap, and real date-interval overlap ratio.
 * 
 * NO HARDCODED values (no 0.91, no 0.85 timeScore, no 0.93 probability).
 * All scores are computed dynamically from input data.
 */
...
export function compareProjectPair(projA, projB) {
  ...
  // Dynamic weight renormalisation based on available signals
